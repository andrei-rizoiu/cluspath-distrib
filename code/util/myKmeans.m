...
while pos_diff > 0.0
  ...
  for d = 1 : length( data(:, 1) );
    ...
    total = total + min_diff;
  end
  
  tst = assignment - oldAssign;
  oldAssign = assignment;
  printf("Total divergence: %.2f, diff: %.2f, same alloc: %d/%d ", total, oldTotal - total, sum(tst == 0), size(data, 1) );
  oldTotal = total;
